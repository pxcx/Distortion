...
#endif
{
    ...
    state->createAndAddParameter("drive", "Drive", "Drive", NormalisableRange<float>(0.f, 1.f, 0.0001), 0.5, nullptr, nullptr);
    state->createAndAddParameter("range", "Range", "Range", NormalisableRange<float>(0.f, 3000.f, 0.0001), 1500.0, nullptr, nullptr);
    state->createAndAddParameter("blend", "Blend", "Blend", NormalisableRange<float>(0.f, 1.f, 0.0001), 0.5, nullptr, nullptr);
    state->createAndAddParameter("volume", "Volume", "Volume", NormalisableRange<float>(0.f, 3.f, 0.0001), 1.5, nullptr, nullptr);
...
void DistortionAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    for (int channel = 0; channel < totalNumInputChannels; ++channel)
    {
        ...
        for (auto sample = 0; sample < buffer.getNumSamples(); sample++) {
            ...
            float signalBlended = cleanSignal * (1.f - blend);
  
            // applying drive and range
            *channelData *= drive * range;

            // smoothing the curve
            float smoothFunction = (2.f / float_Pi) * atan(*channelData);
            *channelData = (((smoothFunction * blend) + signalBlended) / 2.f) * volume;
